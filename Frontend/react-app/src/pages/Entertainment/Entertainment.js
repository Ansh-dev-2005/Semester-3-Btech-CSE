...
export default function Entertainment() {
    ...
    const [EntertainmentPlaces, setEntertainmentPlaces] = useState([]);
        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    fetchEntertainmentPlaces(latitude, longitude);
                },
                (error) => {
    ...
    // Function to fetch nearby places related to Entertainment using the user's location
    const fetchEntertainmentPlaces = async (latitude, longitude) => {
        ...
        const options = {
            ...
            params: {
                ...
                radius: '150000',
                keyword: 'Entertainment Places', // You can adjust the keyword as needed
    ...
    return (
        <div className="p-4">
            {Object.keys(EntertainmentPlaces).map((category, index) => (
                <div key={index} className="mb-4">
                    <h2 className="text-2xl font-bold">{formatCategoryName(category)}</h2>
                    <ul>
                        {EntertainmentPlaces[category].map((place, placeIndex) => (
                                            ...
                                            {place.photos && (
                                                <div>
                                                    ...
                                                    {place.photos.map((photo, photoIndex) => (
                                                        <img
                                                            key={photoIndex}
                                                            src={`https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photo.photo_reference}&key=271302dc76msh353a518ef7811f8p115db8jsn15cfd6685b76`}
