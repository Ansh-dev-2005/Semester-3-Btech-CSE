...
export default function Restaurants() {
        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    fetchRestaurantsPlaces(latitude, longitude);
                },
                (error) => {
    ...
    // Function to fetch nearby places related to Restaurants using the user's location
    const fetchRestaurantsPlaces = async (latitude, longitude) => {
        ...
        const options = {
            ...
            params: {
                ...
                radius: '150000',
                keyword: 'Restaurants Places', // You can adjust the keyword as needed
    ...
    return (
        <div className="p-4">
            {Object.keys(RestaurantsPlaces).map((category, index) => (
                <div key={index} className="mb-4">
                    <h2 className="text-2xl font-bold">{formatCategoryName(category)}</h2>
                    <ul>
                        {RestaurantsPlaces[category].map((place, placeIndex) => (
